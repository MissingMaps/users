    ...
    fetch(`https://osm-stats-production-api.azurewebsites.net/users`)
      ...
      .then(function(data) {
        var usercheck = data.filter(function(element) {
          return (
            element.name.replace(/\s+/g, "-").toLowerCase() === fetch_thisid
          );
        });
        if (usercheck.length > 0) {
          fetch(
            `https://osm-stats-production-api.azurewebsites.net/users/${usercheck[0].id}`
          )
            .then(function(response) {
              if (response.status >= 400) {
              ...
              <div className="Search-Container">
                <div className="Search-Box">
                  ...
                  <div className="Intro-Content">
                    <p>
                      This database began on February 2016, so we haven't picked
                      up any edits by this user yet. If you've just made a
                      contribution, it may take a few minutes to show up. Go do
                      some mapping, then come back and try again!
                    </p>
                    <p>
                    ...
                    <p>
                      ...
                      how to contribute to OpenStreetMap & MissingMaps!
                    </p>
